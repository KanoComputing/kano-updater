...
def test_installed_sources_are_correct(apt_sources_install):
    ...
    assert re.compile(
        r'^deb http://repo.kano.me/archive-stretch/ release main$',
        re.MULTILINE
    ).search(sources)
    assert re.compile(
        r'^deb http://repo.kano.me/archive-stretch/ release-urgent main$',
        re.MULTILINE
    ).search(sources)
    assert re.compile(
        r'^deb http://repo.kano.me/raspberrypi-stretch/ stretch main ui$',
        re.MULTILINE
    ).search(sources)
    assert re.compile(
