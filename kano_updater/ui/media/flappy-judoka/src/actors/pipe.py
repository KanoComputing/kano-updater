...
class Pipe(pygame.sprite.Sprite):
    ...
    def update(self, delta_t):
        ...
        distance = delta_t * self.MOVING_SPEED
        distance = round(distance, 2)  # normalization with 2 decimal points precision
        x -= distance

        self.position = (x, y)
        self.rect.center = self.position

        # returning the distance for score calculation
        return distance
...
class PipeManager(object):
    ...
    PIPE_SPACING_X = 0.32   # distance between two pipes in X
    PIPE_SPACING_Y = 0.32   # distance between top pipe and bottom pipe
    ...
    def __init__(self):
        ...
        self.PIPE_SPACING_X = int(self.PIPE_SPACING_X * self.display_height)
        self.PIPE_SPACING_Y = int(self.PIPE_SPACING_Y * self.display_height)
        ...
        self.no_of_pipes = self.display_width / (self.pipe_width + self.PIPE_SPACING_X) + 2
        ...
        self.distance = 0  # distance traveled by flappy for score calculation
    ...
    def update(self, delta_t):
        ...
        It calls the update for each Pipe, keeps track and returns the
        distance traveled, and moves the first two pipes to the back
        when they fall off the display to the left.
        '''

        # call the update function of every Pipe sprite
        for pipe in self.pipes_list:
            distance = pipe.update(delta_t)
    ...
    def generate_pipes(self):
        ...
        for i in xrange(self.no_of_pipes):
            x = self.PIPE_INIT_X + i * (self.PIPE_SPACING_X + self.pipe_width)
    ...
    def reset_pipes(self):
        ...
        for i in xrange(0, len(self.pipes_list), 2):
            x = self.PIPE_INIT_X + i / 2 * (self.PIPE_SPACING_X + self.pipe_width)
            top_y, bottom_y = self.randomize_pipe_y_positions()
